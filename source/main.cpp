...
vector<string> GetBestMovesFromChessEngine() {
...
void movePiece(void)
{
   ...
   cout << "You can type Z to let computer do the move" << endl;

   getline(cin, move_from);

   //check for the auto move:
   if (move_from == "Z") {
       cout << "you have chosen auto move. Computer will decide the best move for you" << endl;
       auto_move = GetBestMovesFromChessEngine();
   }
   if (auto_move.size() == 2) {
       move_from = auto_move[0];
       move_to = auto_move[1];
       auto_move_flag = 1;
   }
   else {
       createNextMessage("Auto moves were malformed (column and row)\n");
       return;
   }
